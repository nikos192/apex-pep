...
export function Accordion({ items }: AccordionProps) {
  ...
  return (
    <div className="space-y-1 border border-brand-200 rounded-xl overflow-hidden shadow-card">
      {items.map((item, index) => (
        <div key={index} className="border-b border-brand-100 last:border-b-0">
          <button
          ...
          {openIndex === index && (
            <div className="px-6 py-5 bg-white text-brand-700 leading-relaxed border-t border-brand-100">
              {item.children}
            </div>
          )}
        </div>
